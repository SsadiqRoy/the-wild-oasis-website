...
import { differenceInDays, isBefore, isPast, isSameDay, isWithinInterval } from 'date-fns';
...
function DateSelector({ cabin, settings, bookedDates = [] }) {
  ...
  function excludedDates(curDate) {
    const from = new Date(range.from || Date.now());
    const beforeStartDate = isBefore(curDate, from);
    return isPast(curDate) || bookedDates.some((d) => isSameDay(d, curDate)) || beforeStartDate;
  }
